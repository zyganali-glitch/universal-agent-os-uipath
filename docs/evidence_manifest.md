...
| **Data Service memory read/write** | [`backend/uipath_api_connector.py`](../backend/uipath_api_connector.py) | Implemented; write proven live | Reads `CodeSoul`/`MinefieldHistory`; writes approved grants or rejected lessons. Existing strict-real evidence proves a live write. |
