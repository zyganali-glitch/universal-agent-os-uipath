...
## 🚨 PRIMARY DIRECTIVE (EXECUTE IN ORDER)
Whenever a user greets you or proposes a new project (e.g., "Let's start", "I have a project"), you MUST execute these steps in this exact order. **Do not skip Step 1.**
...
2. Run `python backend/labs_smoke_test.py doctor`.
3. Continue only when the read-only check reports `"ready": true`; otherwise explain the missing UiPath entity and stop.
4. Run `python backend/labs_smoke_test.py register`.
5. Read the JSON output and capture the returned `task_id`.
6. Tell the user: "I registered the session and created Action Center task `<task_id>`. Open **UiPath Action Center** (Inbox -> Unassigned), review the 'Phase-0 Alignment Review' task, check the explicit approval field if you approve, and submit the decision."
7. **STOP HERE.** Do not proceed to Step 2 or ask technical questions until the user says they completed the task.
8. After the user responds, run `python backend/labs_smoke_test.py verify`.
9. Read the live UiPath response. Proceed only when it contains `"gate_status": "APPROVED"`. A chat confirmation without API verification is not approval. Pending, missing, or rejected decisions keep the Zero-Leak Lock closed.

### STEP 2: Phase-0 Alignment (Mutual Agreement)
*Only start this step AFTER the user has confirmed UiPath approval from Step 1.*
Ask questions to comprehend the spirit of the project and define contracts (Platform, Billing, i18n, etc.). Ask EXACTLY ONE question at a time.

> [!CAUTION]
> **ZERO-LEAK LOCK:**
> Bypassing Step 1 or Step 2 is an open betrayal to the architecture. No code can be generated until both steps are complete.
